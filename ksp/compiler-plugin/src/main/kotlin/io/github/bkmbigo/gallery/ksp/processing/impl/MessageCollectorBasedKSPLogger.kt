...
class MessageCollectorBasedKSPLogger(
    ...
    private fun convertMessage(message: String, symbol: io.github.bkmbigo.gallery.ksp.symbol.KSNode?): String =
        when (val location = symbol?.location) {
            is io.github.bkmbigo.gallery.ksp.symbol.FileLocation -> "$PREFIX${location.filePath}:${location.lineNumber}: $message"
            is io.github.bkmbigo.gallery.ksp.symbol.NonExistLocation, null -> "$PREFIX$message"
        }

    override fun logging(message: String, symbol: io.github.bkmbigo.gallery.ksp.symbol.KSNode?) {
        recordedEvents.add(Event(CompilerMessageSeverity.LOGGING, convertMessage(message, symbol)))
    }

    override fun info(message: String, symbol: io.github.bkmbigo.gallery.ksp.symbol.KSNode?) {
