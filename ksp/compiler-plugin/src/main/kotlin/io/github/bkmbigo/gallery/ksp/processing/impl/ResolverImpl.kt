...
package io.github.bkmbigo.gallery.ksp.processing.impl
...
import io.github.bkmbigo.gallery.ksp.symbol.impl.binary.*
import io.github.bkmbigo.gallery.ksp.symbol.impl.declarationsInSourceOrder
import io.github.bkmbigo.gallery.ksp.symbol.impl.getInstanceForCurrentRound
import io.github.bkmbigo.gallery.ksp.symbol.impl.java.*
import io.github.bkmbigo.gallery.ksp.symbol.impl.kotlin.*
import io.github.bkmbigo.gallery.ksp.symbol.impl.synthetic.KSConstructorSyntheticImpl
...
import io.github.bkmbigo.gallery.ksp.symbol.impl.synthetic.KSValueParameterSyntheticImpl
...
class ResolverImpl(
     ...
     */
    private val functionAsMemberOfCache: MutableMap<Pair<KSFunctionDeclaration, KSType>, io.github.bkmbigo.gallery.ksp.symbol.KSFunction>
    ...
    fun tearDown() {
        ...
        io.github.bkmbigo.gallery.ksp.common.KSObjectCacheManager.clear()
        instance = null
    }
    ...
    override fun getClassDeclarationByName(name: KSName): io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration? {
    ...
    internal fun mapToJvmSignatureInternal(declaration: KSDeclaration): String? = when (declaration) {
        is io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration -> resolveClassDeclaration(declaration)?.let { typeMapper.mapType(it).descriptor }
    ...
    fun resolveClassDeclaration(classDeclaration: io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration): ClassDescriptor? {
    ...
    fun resolvePropertyAccessorDeclaration(accessor: io.github.bkmbigo.gallery.ksp.symbol.KSPropertyAccessor): PropertyAccessorDescriptor? {
    ...
    fun resolveJavaType(psi: PsiType, parentTypeReference: KSTypeReference? = null): KotlinType {
        ...
        var parent: io.github.bkmbigo.gallery.ksp.symbol.KSNode? = parentTypeReference

        val stack = Stack<io.github.bkmbigo.gallery.ksp.symbol.KSNode>()
        while (parent != null) {
    ...
    fun resolveUserType(type: KSTypeReference): KSType {
        when (type) {
            ...
            is io.github.bkmbigo.gallery.ksp.symbol.impl.java.KSTypeReferenceJavaImpl -> {
        ...
        return when (function.origin) {
            Origin.JAVA -> {
                ...
                psi.throwsList.referencedTypes.asSequence()
                    .map { io.github.bkmbigo.gallery.ksp.symbol.impl.java.KSTypeReferenceJavaImpl.getCached(it, function).resolve() }
            }
            Origin.KOTLIN -> {
    ...
    private fun KotlinType.isSubtypeOf(declaration: io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration): Boolean {
    ...
    @KspExperimental
    internal fun mapToJvmSignature(accessor: io.github.bkmbigo.gallery.ksp.symbol.KSPropertyAccessor): String {
    ...
    private fun KSTypeReference.findJvmSuppressWildcards(): Boolean? {
        var candidate: io.github.bkmbigo.gallery.ksp.symbol.KSNode? = this
...
internal fun KSAnnotated.findAnnotationFromUseSiteTarget(): Sequence<KSAnnotation> {
    return when (this) {
        is io.github.bkmbigo.gallery.ksp.symbol.KSPropertyGetter -> (this.receiver as? KSDeclarationImpl)?.let {
            it.originalAnnotations.asSequence().filter { it.useSiteTarget == AnnotationUseSiteTarget.GET }
        }
        is io.github.bkmbigo.gallery.ksp.symbol.KSPropertySetter -> (this.receiver as? KSDeclarationImpl)?.let {
