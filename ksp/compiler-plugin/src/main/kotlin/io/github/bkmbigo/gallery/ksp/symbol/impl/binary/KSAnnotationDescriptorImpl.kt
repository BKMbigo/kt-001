...
import io.github.bkmbigo.gallery.ksp.symbol.impl.findPsi
import io.github.bkmbigo.gallery.ksp.symbol.impl.java.KSAnnotationJavaImpl
import io.github.bkmbigo.gallery.ksp.symbol.impl.kotlin.KSErrorType
import io.github.bkmbigo.gallery.ksp.symbol.impl.kotlin.KSValueArgumentLiteImpl
...
import io.github.bkmbigo.gallery.ksp.getClassDeclarationByName
...
class KSAnnotationDescriptorImpl private constructor(
    ...
    companion object : KSObjectCache<Pair<AnnotationDescriptor, io.github.bkmbigo.gallery.ksp.symbol.KSNode?>, KSAnnotationDescriptorImpl>() {
        fun getCached(descriptor: AnnotationDescriptor, parent: io.github.bkmbigo.gallery.ksp.symbol.KSNode?) = cache.getOrPut(Pair(descriptor, parent)) {
...
private fun <T> ConstantValue<T>.toValue(parent: io.github.bkmbigo.gallery.ksp.symbol.KSNode): Any? = when (this) {
    ...
    is EnumValue -> value.first.findKSClassDeclaration()?.declarations?.find {
        it is io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration && it.classKind == ClassKind.ENUM_ENTRY &&
            it.simpleName.asString() == value.second.asString()
    }?.let { (it as io.github.bkmbigo.gallery.ksp.symbol.KSClassDeclaration).asStarProjectedType() }
